<template>
  <div class="modal-overlay" v-if="isOpen" @click.self="$emit('close')" tabindex="-1">
    <div class="modal-card modal-lg course-detail-popup">
      <!-- Close Button -->
      <button class="modal-close-icon" @click="$emit('close')" :title="content?.modalTexts?.courseDetail?.closeTooltip || 'Close Popup (Esc)'">✕</button>
      ...
      <div class="detail-popup-header">
        ...
        <div class="detail-header-content">
          ...
          <div class="detail-popup-sub">{{ content?.modalTexts?.courseDetail?.subheading || 'IT Solutions & Production Internship Track 2026 • Live Project Lab' }}</div>
        </div>
      </div>
      ...
      <div class="metric-stats-grid">
        ...
        <div class="metric-stat-card stat-earning">
          <div class="metric-stat-top">
            ...
            <span class="metric-stat-tag">{{ content?.modalTexts?.courseDetail?.earningTag || 'Earning Potential' }}</span>
          </div>
          <div class="metric-stat-value text-gradient-gold">
            {{ track.earningPotential?.fresher || '₹4.5 - ₹7.5 LPA' }}
          </div>
          <div class="metric-stat-detail">
            <div><strong>{{ content?.modalTexts?.courseDetail?.fresherLabel || 'Fresher Salary:' }}</strong> {{ track.earningPotential?.fresher }}</div>
            <div><strong>{{ content?.modalTexts?.courseDetail?.experiencedLabel || 'Experienced (2-3+ Yrs):' }}</strong> {{ track.earningPotential?.experienced }}</div>
            <div><strong>{{ content?.modalTexts?.courseDetail?.freelanceLabel || 'Freelance / Remote:' }}</strong> {{ track.earningPotential?.freelance }}</div>
          </div>
        </div>
        ...
        <div class="metric-stat-card stat-placement">
          <div class="metric-stat-top">
            <span class="metric-stat-icon">🎯</span>
            <span class="metric-stat-tag">{{ content?.modalTexts?.courseDetail?.successRateTag || 'Job Success Rate' }}</span>
          </div>
          <div class="metric-stat-value text-gradient">
            {{ track.jobPlacementRate?.percentage || 95 }}{{ content?.modalTexts?.courseDetail?.placementSuffix || '% Placement' }}
          </div>
          <div class="metric-stat-detail">
        ...
        <div class="metric-stat-card stat-hours">
          <div class="metric-stat-top">
            ...
            <span class="metric-stat-tag">{{ content?.modalTexts?.courseDetail?.hoursTag || 'Learning Hours' }}</span>
          </div>
          <div class="metric-stat-value" style="color: var(--color-ai-cyan);">
            {{ track.learningHours?.totalHours || '180+ Hours' }}
          </div>
          <div class="metric-stat-detail">
            <div><strong>{{ content?.modalTexts?.courseDetail?.weeklyHoursLabel || 'Weekly Commitment:' }}</strong> {{ track.learningHours?.weeklyHours || '12-15 Hrs/Wk' }}</div>
            <div><strong>{{ content?.modalTexts?.courseDetail?.liveLabHoursLabel || 'Live Lab Coding:' }}</strong> {{ track.learningHours?.liveLabHours || '120+ Hrs' }}</div>
            <div><strong>{{ content?.modalTexts?.courseDetail?.scheduleLabel || 'Schedule:' }}</strong> {{ track.learningHours?.schedule || 'Daily 2 Hours Interactive Lab' }}</div>
          </div>
        </div>
        ...
        <div class="metric-stat-card stat-roles">
          <div class="metric-stat-top">
            <span class="metric-stat-icon">💼</span>
            <span class="metric-stat-tag">{{ content?.modalTexts?.courseDetail?.targetRolesTag || 'Target Job Roles' }}</span>
          </div>
      ...
      <div class="detail-section-block">
        <h3 class="detail-section-heading">
          <span>🔬</span> {{ content?.modalTexts?.courseDetail?.techScopeHeading || 'Details About Technology & Industry Scope' }}
        </h3>
        ...
        <div class="market-insight-box" v-if="track.earningPotential?.overview">
          ...
          <div>
            <strong>{{ content?.modalTexts?.courseDetail?.marketOverviewLabel || 'Current Market Scenario:' }}</strong> {{ track.earningPotential.overview }}
          </div>
        </div>
      </div>
      ...
      <div class="detail-section-block">
        <h3 class="detail-section-heading">
          <span>💻</span> {{ content?.modalTexts?.courseDetail?.techLearnedHeading || 'Technologies & Tools You Will Learn' }}
        </h3>
      ...
      <div class="detail-section-block" v-if="track.liveProjects && track.liveProjects.length">
        <h3 class="detail-section-heading">
          <span>🚀</span> {{ content?.modalTexts?.courseDetail?.liveProjectsHeading || 'Real-World Production Projects You Will Build' }}
        </h3>
        <div class="projects-detail-grid">
          <div class="project-detail-card" v-for="(proj, pIdx) in track.liveProjects" :key="proj.name">
            <div class="project-number-badge">{{ (content?.modalTexts?.courseDetail?.projectBadgePrefix || 'Project 0') + (pIdx + 1) }}</div>
      ...
      <div class="detail-section-block" v-if="track.curriculumHighlights && track.curriculumHighlights.length">
        <h3 class="detail-section-heading">
          <span>📚</span> {{ content?.modalTexts?.courseDetail?.curriculumHeading || 'Structured Curriculum Milestones' }}
        </h3>
      ...
      <div class="perks-guarantee-banner">
        ...
        <div>
          <div style="font-weight: 800; font-size: 1rem; color: var(--text-main); margin-bottom: 0.25rem;">
            {{ content?.modalTexts?.courseDetail?.guaranteeTitle || 'Verified Internship Certification + Corporate Letter of Recommendation (LOR)' }}
          </div>
          <div style="font-size: 0.85rem; color: var(--text-muted);">
            {{ content?.modalTexts?.courseDetail?.guaranteeDesc || 'Includes 100% placement assistance, resume masterclass, mock technical interviews, and GitHub production code portfolio review.' }}
          </div>
        </div>
      </div>
      ...
      <div class="detail-modal-footer">
        <button type="button" class="btn-secondary" @click="$emit('close')">
          {{ content?.modalTexts?.courseDetail?.backBtn || 'Back to Tracks' }}
        </button>
        <button type="button" class="btn-primary proceed-btn" @click="$emit('proceed-register', track)">
          <span>{{ content?.modalTexts?.courseDetail?.proceedBtn || 'Proceed to Registration 📝 →' }}</span>
        </button>
      </div>
    </div>
  </div>
